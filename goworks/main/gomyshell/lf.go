...
func init(){
	...
	if len(flag.Args()) != 0{
		if flag.Arg(0) != "_"{
			diresub.contain = append(diresub.contain, my.CompileAll(reg.Split(flag.Arg(0), -1))...)
			rFlag = true
		}
		if flag.Arg(1) != "_"{
			namesub.contain = append(namesub.contain, my.CompileAll(reg.Split(flag.Arg(1), -1))...)
		}
		if !dFlag && flag.Arg(2) != "_"{
			linesub.contain = append(linesub.contain, my.CompileAll(reg.Split(flag.Arg(2), -1))...)
		}
	}
	//フラグの有無判定
		...
		if lineF {
			...
			if s == "open"{
				...
				if runtime.GOOS == "windows"{
					exec.Command("cmd", "/k", fullName).Run()
				}
